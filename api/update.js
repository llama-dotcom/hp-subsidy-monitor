    ...
    try {
      ...
      if (mfgArticles.length > 0) {
        ...
        const mfgCompletion = await groqChat(groq, [
            ...
            { role: 'user', content: `News articles potentially about heat pump manufacturers:\n\n${mfgList}\n\n=== ACCEPT (must be about a heat pump product from a SPECIFIC named brand) ===\n• New HEAT PUMP model launch or product update (e.g., "LG launches Therma V R290", "Vaillant aroTHERM plus")\n• Heat pump product range expansion or new heat pump series\n• Heat pump factory news (new plant, capacity expansion for heat pump production)\n• Heat pump related M&A / partnerships between HP manufacturers\n• Heat pump sales volumes or market share by brand\n• Multiple brands competing or compared in heat pump market (→ use Mixed)\n\n=== REJECT ===\n• Products that are NOT heat pumps (chillers, controllers, superheat devices, AHUs, refrigeration)\n• Articles about a brand's NON-heat-pump business (automotive, appliances, general corporate)\n• Subsidy / grant / funding programs\n• Government policy / regulation\n• Generic market forecasts not tied to a specific brand\n• Installation tips / consumer guides\n• Training centers / events (unless launching a new HP product there)\n\nFor each ACCEPTED article identify the MANUFACTURER NAME (e.g., "Bosch", "Daikin", "Vaillant", "LG", "NIBE").\nIf an article discusses MULTIPLE manufacturers together (e.g., "Bosch, Vaillant and Viessmann face competition from Chinese brands"), set manufacturer to "Mixed".\n\nIMPORTANT: If multiple articles describe the SAME event from different outlets, merge into ONE summary.\n\nReturn: {"items": [{"title": "BrandName: headline max 80 chars (ALWAYS start with brand name and colon, e.g. 'Daikin: Expands Rental Fleet'. For Mixed: 'Mixed: headline')", "description": "3-5 sentences summarizing the article content. Include specific facts from the excerpt: numbers, model names, financial figures, locations. Never invent information not found in the headline or excerpt.", "impact": "critical|medium|info", "manufacturer": "ManufacturerName or Mixed", "original_index": number}]}\n\nIf NO articles match ACCEPT criteria: {"items": []}` }
    ...
    for (const country of topCountries) {
      try {
        ...
        const completion = await groqChat(groq, [
            { role: 'system', content: 'You analyze news about heat pump subsidies and policy. Return JSON only. Base summaries on article excerpts. Never invent details not in the excerpt.' },
            { role: 'user', content: `News articles about heat pumps in ${country.name}:\n\n${articleList}\n\n=== STRICT COUNTRY FILTER ===\nONLY include articles that are genuinely about ${country.name}. REJECT articles that:\n• Are actually about a DIFFERENT country (e.g., an article about France appearing in a search for Bulgaria)\n• Are generic global/EU market forecasts not specific to ${country.name}\n• Only mention ${country.name} in passing while focusing on another country\n\n=== DEDUP RULE ===\nIf multiple articles describe the SAME event, merge into ONE summary.\n\nReturn: {"items": [{"title": "headline max 80 chars", "description": "3-5 sentences. Include specific facts from the excerpt: subsidy amounts in EUR, program names, eligibility criteria, dates, percentages. If the article is in a foreign language, translate key facts to English.", "impact": "critical|medium|info", "original_index": number}]}\n\nIf NO articles pass the country filter: {"items": []}` }
